...
// files to delete from the Classic Shell folder
static const wchar_t *g_InstalledFiles[]=
{
	...
	L"ClassicIE_64.exe",
	L"ClassicShell.chm",
	L"ClassicShellReadme.rtf",
	L"ClassicShellUpdate.exe",
...
// skin files to delete from the Classic Shell\Skins folder
...
// shortcuts to delete from the Programs\Classic Shell folder
static const wchar_t *g_InstalledShortcuts[]=
{
	L"Classic Explorer Settings.lnk",
	L"Classic IE Settings.lnk",
	L"Classic Shell Help.lnk",
	L"Classic Shell Readme.lnk",
	L"Classic Shell Update.lnk",
...
LRESULT CUninstallDlg::OnInitDialog( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	{
		HKEY hkey=NULL;
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,L"SOFTWARE\\IvoSoft\\ClassicShell",0,KEY_READ|KEY_QUERY_VALUE|KEY_WOW64_64KEY,&hkey)==ERROR_SUCCESS)
		{
			DWORD type=0;
	...
	if (path[0]==0)
	{
		...
		Strcpy(defaultPath,_countof(defaultPath),bWow64?L"%ProgramW6432%\\Classic Shell":L"%ProgramFiles%\\Classic Shell");
	...
	if (m_bEmptyPath)
		SetDlgItemText(IDC_STATICPATH,L"The tool failed to locate the Classic Shell installation folder. Use the Browse button to locate the correct folder if you want the files in it to be deleted.");
	else
	{
...
LRESULT CUninstallDlg::OnBrowse( WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled )
{
	...
	{
		pDialog->SetTitle(L"Locate the Classic Shell install folder");
...
LRESULT CResultsDlg::OnInitDialog( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	if (m_bErrors)
	{
		SetDlgItemText(IDC_STATICRESULT,L"The Classic Shell removal tool encountered some errors. Please, restart your computer and try again. If the problem is not resolved"
			L" seek help in the Classic Shell forums: www.classicshell.net/forum. Copy the following report and post it in the forum. The report is also saved in a file ClassicShellReport.txt on your desktop.");
	}
	else if (m_bReboot)
	{
		SetDlgItemText(IDC_STATICRESULT,L"The Classic Shell removal tool finished. A restart is required to complete the process."
			L"Copy the following report for your records. The report is also saved in a file ClassicShellReport.txt on your desktop.");
	}
	else
	{
		SetDlgItemText(IDC_STATICRESULT,L"The Classic Shell removal tool finished."
			L"Copy the following report for your records. The report is also saved in a file ClassicShellReport.txt on your desktop.");
	}
...
{
	wchar_t path[_MAX_PATH]=L"%USERPROFILE%\\Desktop\\ClassicShellReport.txt";
...
static bool RemoveRegistryKeys( bool bPin )
{
	...
	if (error==ERROR_SUCCESS)
	{
		...
		if (RegOpenKeyEx(HKEY_CLASSES_ROOT,L"CLSID\\" EXPLORER_CLSID L"\\" EMULATION_KEY,0,KEY_READ|KEY_WOW64_64KEY,&hkey)==ERROR_SUCCESS)
		{
			...
			MessageBox(NULL,message,L"Classic Shell Uninstaller",MB_OK|MB_ICONERROR);
			return false;
		}
	}
...
static void DeleteSettings( HKEY root, const wchar_t *rootName, const wchar_t *keyName )
{
	...
	if (error==ERROR_SUCCESS)
	{
		...
		error=RegDeleteTree2(hkey,L"ClassicShell");
		if (error==ERROR_SUCCESS)
			LogMessage(-1,L"Deleting registry key %s\\%s\\ClassicShell",rootName,keyName);
		else if (error!=ERROR_FILE_NOT_FOUND)
			LogMessage(error,L"Failed to delete registry key %s\\%s\\ClassicShell.",rootName,keyName);
...
static void ManualUninstallInternal( void )
{
	...
	LogMessage(-1,L"Classic Shell Utility - removing Classic Shell from the system\r\n\r\n");
	...
	DeleteRegKeySOFTWARE(L"IvoSoft\\ClassicShell",bIsWow64);
	...
	DeleteInstallerKey(HKEY_CLASSES_ROOT,L"HKEY_CLASSES_ROOT",L"Installer\\Features",L"ClassicShell",L"");
	DeleteInstallerKey(HKEY_CLASSES_ROOT,L"HKEY_CLASSES_ROOT",L"Installer\\Products",L"ProductName",L"Classic Shell");
	DeleteInstallerKey(HKEY_LOCAL_MACHINE,L"HKEY_LOCAL_MACHINE",L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall",L"DisplayName",L"Classic Shell");
	...
	if (dlg.GetRemoveUserSettings() || dlg.GetRemoveAllUserSettings())
	{
		...
		wchar_t local[_MAX_PATH]=L"%LOCALAPPDATA%\\ClassicShell";
		DoEnvironmentSubst(local,_countof(local));
		wchar_t roaming[_MAX_PATH]=L"%APPDATA%\\ClassicShell";
	...
	if (dlg.GetRemoveAllUserSettings())
	{
		...
		for (std::vector<CString>::const_iterator it=userNames.begin();it!=userNames.end();++it)
		{
			...
			Sprintf(local,_countof(local),L"%s\\%s\\AppData\\Local\\ClassicShell",usersFolder,*it);
			wchar_t roaming[_MAX_PATH];
			Sprintf(roaming,_countof(roaming),L"%s\\%s\\AppData\\Roaming\\ClassicShell",usersFolder,*it);
			DeleteUserFiles(local,roaming);
	...
	// delete program data
	{
		wchar_t dataPath[_MAX_PATH]=L"%ALLUSERSPROFILE%\\ClassicShell";
	...
	// delete start menu shortcuts
	{
		...
		if (SUCCEEDED(SHGetKnownFolderPath(FOLDERID_CommonPrograms,0,NULL,&pPath)))
		{
			...
			Sprintf(startMenuPath,_countof(startMenuPath),L"%s\\Classic Shell",(const wchar_t*)pPath);
...
///////////////////////////////////////////////////////////////////////////////
	...
	{
		MessageBox(NULL,L"It is important that you restart as soon as possible. If you attempt to install another software before then it may get corrupted.",L"Classic Shell Uninstaller",MB_OK|MB_ICONWARNING);
	}
}
