...
static void WriteLogFile( FILE *f )
{
	...
	{
		CRegKey regKey;
		DWORD err=regKey.Open(HKEY_LOCAL_MACHINE,L"Software\\IvoSoft\\ClassicShell",KEY_READ|KEY_WOW64_64KEY);
		if (err!=ERROR_SUCCESS)
		{
			fwprintf(f,L"\tFailed to read HKLM\\Software\\IvoSoft\\ClassicShell - 0x%08X\r\n",err);
		}
		else
		{
			...
			{
				fwprintf(f,L"\tClassic Shell path: '%s'\r\n",csPath);
...

	{
		...
		Strcpy(path,_countof(path),L"%ALLUSERSPROFILE%\\ClassicShell");
	...
	Sprintf(fname,_countof(fname),L"%s\\ClassicShellUpdate.exe",csPath);
	...
	if (bClassicUpdate)
	{
		fwprintf(f,L"  Classic Shell Update\r\n\r\n");
	}
...
bool ExtractUtility64( const wchar_t *fname, wchar_t *exe )
{
	...
	if (!hResInfo)
	{
		fwprintf(f,L"Error extracting ClassicShellUtility64.exe\r\n");
		fclose(f);
		return false;
	}
	...
	bool res=false;
	Strcpy(exe,_MAX_PATH,L"%TEMP%\\ClassicShellUtility64.exe");
