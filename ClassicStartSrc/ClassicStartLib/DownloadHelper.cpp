...
static DWORD WINAPI ThreadVersionCheck( void *param )
{
	...
	CRegKey regKey;
	if (regKey.Open(HKEY_CURRENT_USER,L"Software\\PassionateCoder\\ClassicStart")!=ERROR_SUCCESS)
	{
...
VersionData::TLoadResult VersionData::Load( const wchar_t *fname, bool bLoadFlags )
{
	...
	{
		CRegKey regKeyLng;
		if (regKeyLng.Open(HKEY_LOCAL_MACHINE,L"Software\\PassionateCoder\\ClassicStart",KEY_READ|KEY_WOW64_64KEY)==ERROR_SUCCESS)
		{
