...
*/
	...
	if (newestProgram.QuadPart || newestApp.QuadPart)
	{
		CRegKey regKey;
		ULONGLONG val1, val2;
		if (regKey.Open(HKEY_CURRENT_USER,L"Software\\PassionateCoder\\ClassicStartMenu",KEY_READ)==ERROR_SUCCESS)
		{
...
void CItemManager::LoadOldItems( void )
{
	...
	CRegKey regItems;
	if (regItems.Open(HKEY_CURRENT_USER,L"Software\\PassionateCoder\\ClassicStartMenu",KEY_READ)==ERROR_SUCCESS)
	{
		ULONG size=0;
...
void CItemManager::AddOldItems( const std::vector<unsigned> &hashes )
{
	...
	CRegKey regItems;
	if (regItems.Open(HKEY_CURRENT_USER,L"Software\\PassionateCoder\\ClassicStartMenu")!=ERROR_SUCCESS)
		regItems.Create(HKEY_CURRENT_USER,L"Software\\PassionateCoder\\ClassicStartMenu");
