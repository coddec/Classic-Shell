...
[fa-IR] - Persian
Menu.Programs = &برنامه‌ها
Menu.Apps = برنامه‌ها
Menu.AllPrograms = همه برنامه‌ها
Menu.Back = عقب
Menu.Favorites = علا&قه‌مندی‌ها
Menu.Documents = ا&سناد
Menu.Settings = تن&ظیمات
Menu.Search = &جستجو
Menu.SearchBox = جستجو
Menu.SearchPrograms = جستجوی برنامه‌ها و پرونده‌ها
...
Menu.Restart = &راه‌اندازی مجدد
Menu.ShutdownUpdate = به‌روزرسانی و خاموش کردن
Menu.RestartUpdate = به‌روزرسانی و راه‌اندازی مجدد
Menu.Sleep = خ&واب
...
Menu.SearchFiles = برای &پرونده‌ها و پوشه‌ها...
Menu.SearchPrinter = برای &چاپگر
Menu.SearchComputers = برای &رایانه‌ها
Menu.UserFilesTip = شامل پوشه‌های اسناد، تصاویر، موسیقی‌ها و پرونده‌های دیگری است که به شما تعلق دارد.
Menu.UserDocumentsTip = شامل نامه‌ها، اخبار و اسناد و پرونده‌های دیگر است.
Menu.UserPicturesTip = شامل عکس‌های دیجیتالی، تصاویر و پرونده‌های گرافیکی است.
Menu.UserMusicTip = شامل موسیقی‌ها و دیگر پرونده‌های صوتی است.
Menu.UserVideosTip = شامل فیلم‌ها و دیگر پرونده‌های ویدئویی است.
Menu.NetworkTip = ‏‏اتصالات شبکه موجود روی رایانه را نمایش می‌دهد و به شما در ایجاد اتصالات جدید کمک می کند
...
Menu.DocumentsLibTip = دسترسی به نامه‌ها، گزارش‌ها، یادداشت‌ها، و انواع دیگر مدارک.
Menu.MusicLibTip = پخش موسیقی و پرونده‌های صوتی دیگر.
Menu.PicturesLibTip = مشاهده و سازماندهی تصاویر دیجیتالی.
Menu.VideosLibTip = تماشای فیلم‌های خانوادگی و سایر فیلم‌های دیجیتالی.
Menu.RecordingsLibTip = تماشای برنامه‌های تلویزیونی ضبط شده در رایانه شما.
Menu.DownloadTip = یافتن بارگیری‌های اینترنتی و پیوند به وب‌سایت‌های دلخواه.
Menu.HomegroupTip = به کتابخانه‌ها و پوشه‌هایی که دیگران به اشتراک گذاشته‌اند در گروه خانگی خود دسترسی پیدا کنید.
Menu.RunTip = یک برنامه، پوشه، سند یا وب‌سایت را باز می‌کند.
Menu.HelpTip = پیدا کردن عناوین راهنما، آموزش، رفع اشکال، و خدمات پشتیبانی دیگر.
Menu.ProgramsTip = فهرستی از برنامه‌های شما را باز می‌کند.
Menu.SearchFilesTip = جستجو برای سندها، موسیقی‌ها، تصاویر، پست الکترونیکی و موارد دیگر.
Menu.GamesTip = بازی کردن و مدیریت بازی‌های رایانه شما.
Menu.SecurityTip = راه‌اندازی گزینه‌های امنیتی ویندوز برای تغییر رمز ورود، تعویض کاربر یا شروع کنترل‌گر فعالیت‌ها.
Menu.SearchComputersTip = جستجو برای رایانه‌ها در شبکه
Menu.SearchPrintersTip = جستجو برای چاپگر
Menu.AdminToolsTip = پیکربندی تنظیمات مدیریت برای رایانه شما.
Menu.ShutdownTip = همه برنامه‌های باز را می‌بندد، ویندوز را خاموش می‌کند و سپس رایانه شما را خاموش می‌کند.
Menu.RestartTip = همه برنامه‌های باز را می‌بندد، ویندوز را خاموش می‌کند و سپس ویندوز را مجدداً راه‌اندازی می‌کند.
Menu.SleepTip = جلسه شما را در حافظه نگه می‌دارد و رایانه را در حالت مصرف برق کمتر قرار می‌دهد تا شما سریعاً بتوانید به کار خود برگردید.
Menu.HibernateTip = جلسه شما را ذخیره و رایانه را خاموش می‌کند. وقتی که رایانه را روشن می‌کنید، ویندوز جلسه شما را باز می‌گرداند.
Menu.LogOffTip = ‏‏برنامه‌ها را ببندید و از سیستم خارج شوید.
Menu.DisconnectTip = جلسه شما را قطع می‌کند. می‌توانید هنگامی که دوباره به سیستم وارد می‌شوید مجدداً به این جلسه وصل شوید.
Menu.LockTip = این رایانه را قفل کنید.
Menu.UndockTip = رایانه کیفی یا نوت‌بوک خود را از محل استقرار جدا کنید.
Menu.SwitchUserTip =  تعویض کاربران بدون بستن برنامه‌ها.
Menu.Empty = (خالی)
Menu.Features = برنامه‌ها و ویژگی‌ها
Menu.FeaturesTip = برنامه‌های رایانه خود را تغییر داده یا پاک کنید.
...
Search.CategoryVideos = فیلم‌ها
Search.CategoryFiles = پرونده‌ها
...
Menu.NoMatch = Aucun élément ne correspond à votre recherche.
...
Menu.UserFilesTip = Contient des dossiers pour les documents, les images, la musique et d'autres fichiers qui vous appartiennent.
Menu.UserDocumentsTip = Ouvrez vos lettres, rapports et autres documents et fichiers.
...
Menu.ControlPanelTip = Modifiez les paramètres et personnalisez la fonctionnalité de votre ordinateur.
Menu.DocumentsLibTip = Accédez aux lettres, rapports et notes et à d’autres types de documents.
Menu.MusicLibTip = Écoutez de la musique et des fichiers audio.
Menu.PicturesLibTip = Affichez et organisez vos images.
Menu.VideosLibTip = Regardez des films et d’autres vidéos numériques chez vous.
Menu.RecordingsLibTip = Regardez les programmes TV enregistrés sur votre ordinateur.
Menu.DownloadTip = Recherchez des téléchargements Internet et des liens vers les sites Web favoris.
Menu.HomegroupTip = Accédez aux bibliothèques et aux dossiers partagés par les autres membres de votre groupe résidentiel.
Menu.RunTip = Ouvrez un programme, un dossier, un document ou un site Web.
Menu.HelpTip = Recherchez des rubriques d’aide, des didacticiels, des informations de dépannage et d’autres services d’assistance.
Menu.ProgramsTip = Affichez tous les programmes présents sur votre ordinateur.
Menu.SearchFilesTip = Recherchez des documents, des morceaux de musique, des images, des messages et bien plus encore.
Menu.GamesTip = Jouez des parties et gérez les jeux de votre ordinateur.
Menu.SecurityTip = Lancez les Options de sécurité Windows pour modifier le mot de passe, changer d’utilisateur ou ouvrir le gestionnaire des tâches.
Menu.SearchComputersTip = Rechercher des ordinateurs sur le réseau
Menu.SearchPrintersTip = Rechercher une imprimante
Menu.AdminToolsTip = Configurez les paramètres d’administration de votre ordinateur.
Menu.ShutdownTip = Fermez tous les programmes ouverts, arrêtez Windows et éteignez l'ordinateur.
Menu.RestartTip = Fermez tous les programmes ouverts, arrêtez Windows puis redémarrez Windows.
Menu.SleepTip = Conservez votre session en mémoire en plaçant l’ordinateur en mode de consommation réduite, pour vous permettre de reprendre rapidement votre travail.
Menu.HibernateTip = Enregistrez votre session et éteignez l'ordinateur. Lorsque vous allumerez l'ordinateur, votre session sera restaurée.
Menu.LogOffTip = Fermez les programmes et votre session.
Menu.DisconnectTip = Déconnectez votre session. Vous pourrez vous reconnecter à cette session quand vous ouvrirez une nouvelle session.
Menu.LockTip = Verrouillez cet ordinateur.
Menu.UndockTip = Retirez votre ordinateur portable de sa station d’accueil.
Menu.SwitchUserTip = Changez d'utilisateur sans fermer les programmes.
Menu.Empty = (Vide)
Menu.Features = Programmes et fonctionnalités
Menu.FeaturesTip = Désinstaller ou modifier des programmes sur votre ordinateur.
...
Menu.LogoffPrompt = Voulez-vous vraiment fermer votre session ?
...
Menu.NoMatch = Vyhľadávanie nenašlo zhodu.
Menu.MoreResults = Zobraziť viac výsledkov
Menu.Help = &Pomoc a technická podpora
Menu.Run = &Spustiť...
Menu.Logoff = Odhlásiť &používateľa %s...
Menu.SwitchUser = &Prepnúť používateľa
Menu.Lock = &Uzamknúť
Menu.LogOffShort = &Odhlásiť
Menu.Undock = &Odpojiť z dokovacej stanice
...
Menu.PCSettings = Nastavenia počítača
Menu.Security = Zabezpečenie systému Windows
Menu.Network = &Sieťové pripojenia
Menu.Printers = &Tlačiareň
Menu.Taskbar = &Panel úloh a ponuka Štart
...
Menu.UserDocumentsTip = Obsahuje listy, hlásenia a ďalšie súbory a dokumenty.
Menu.UserPicturesTip = Obsahuje digitálne fotografie, obrázky a grafické súbory.
Menu.UserMusicTip = Obsahuje hudobné a iné zvukové súbory.
Menu.UserVideosTip = Obsahuje filmy a iné videosúbory.
Menu.NetworkTip = Zobrazí existujúce sieťové pripojenia a pomôže vytvoriť nové pripojenia.
Menu.PrintersTip = Pridá, odstráni a nastaví lokálne alebo sieťové tlačiarne.
...
Menu.MusicLibTip = Umožňuje prehrávať hudobné a iné zvukové súbory.
...
Menu.DownloadTip = Umožňuje hľadanie súborov na stiahnutie z Internetu a odkazov na obľúbené weby.
Menu.HomegroupTip = Pristupujte ku knižniciam a priečinkom zdieľanými ďalšími používateľmi v domácej skupine.
...
Menu.ProgramsTip = Zobrazí zoznam programov v počítači.
Menu.SearchFilesTip = Umožňuje vyhľadávať dokumenty, hudbu, obrázky, e-maily a ďalšie.
Menu.GamesTip = Umožňuje hrať a spravovať hry v počítači.
Menu.SecurityTip = Spustí možnosti zabezpečenia systému Windows, v ktorých je možné zmeniť heslo, prepnúť používateľa alebo spustiť Správcu úloh.
Menu.SearchComputersTip = Hľadať počítače v sieti
Menu.SearchPrintersTip = Hľadať tlačiarne
...
Menu.SleepTip = Ponechá reláciu v pamäti a uvedie počítač do režimu nízkej spotreby, takže je možné rýchlo pokračovať v práci.
Menu.HibernateTip = Uloží reláciu a vypne počítač. Po zapnutí počítače systém Windows reláciu obnoví.
Menu.LogOffTip = Umožňuje ukončiť programy a odhlásiť používateľa.
Menu.DisconnectTip = Odpojí vašu reláciu. K relácii sa môžete opäť pripojiť pri ďalšom prihlásení.
Menu.LockTip = Uzamkne počítač.
Menu.UndockTip = Odpojí prenosný počítač z dokovacej stanice.
Menu.SwitchUserTip = Prepne používateľa bez zatvorenia programov.
Menu.Empty = (Prázdne)
Menu.Features = Programy a súčasti
Menu.FeaturesTip = Odinštaluje alebo zmení programy v počítači.
Menu.SearchPeople = &Ľudia...
Menu.SortByName = Zoradiť podľa &názvu
Menu.Open = &Otvoriť
Menu.OpenAll = &Otvoriť profil všetkých používateľov
Menu.Explore = &Preskúmať
Menu.ExploreAll = &Preskúmať súbory všetkých používateľov
Menu.MenuSettings = Nastavenia
Menu.MenuHelp = Pomocník
Menu.MenuExit = Ukončiť
Menu.LogoffTitle = Odhlásiť sa zo systému Windows
Menu.LogoffPrompt = Skutočne sa chcete odhlásiť?
Menu.LogoffYes = &Odhlásiť
Menu.LogoffNo = &Nie
Menu.RenameTitle = Premenovať
Menu.RenamePrompt = &Nový názov:
Menu.RenameOK = OK
Menu.RenameCancel = Zrušiť
Menu.Organize = Usporiadať ponuku Štart
Menu.Expand = &Rozbaliť
Menu.Collapse = &Zbaliť
Menu.NewFolder = Nový priečinok
Menu.NewShortcut = Nový odkaz
Menu.AutoArrange = &Zoradiť automaticky
Menu.ActionOpen = Otvoriť
Menu.ActionClose = Zavrieť
Menu.ActionExecute = Spustiť
Menu.RemoveList = &Odstrániť z tohto zoznamu
Menu.RemoveAll = Vymazať zoznam &posledných súborov
Menu.Explorer = Prieskumník Windows
Menu.Start = Štart
Menu.StartScreen = Úvodná obrazovka
...
Search.CategoryPCSettings = Nastavenia počítača
Search.CategoryPrograms = Programy
Search.CategoryDocuments = Dokumenty
Search.CategoryMusic = Hudba
Search.CategoryPictures = Obrázky
...
JumpList.Pin = &Pripnúť do tohoto zoznamu
JumpList.Unpin = &Zrušiť pripnutie v tomto zozname
JumpList.Remove = &Odstrániť z tohoto zoznamu
JumpList.PinTip = Pripnúť do tohoto zoznamu
